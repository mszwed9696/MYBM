...
def extract_character_image(file_path):
    """Extract character image src from a page.tsx file"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()

        # Look for image src in character sections
        # Pattern: src="/characters/[filename].png"
        match = re.search(r'src="(/characters/[^"]+\.(?:png|jpg|jpeg|webp))"', content)
        if match:
            return match.group(1).replace('/characters/', '')

        return "No character image found"
...
print("=" * 80)
print("CURRENT CHARACTER IMAGES BY PAGE")
print("=" * 80)
print()
...
if homepage.exists():
    image = extract_character_image(homepage)
    print(f"/                                  → {image}")
print()
...
if services_dir.exists():
    ...
    for service_dir in service_pages:
        ...
        if page_file.exists():
            ...
            image = extract_character_image(page_file)
...
for page_name, page_path in other_pages:
    ...
    if full_path.exists():
        image = extract_character_image(full_path)
...
if results_hero.exists():
    image = extract_character_image(results_hero)
